...
### 3.1 精度向上の進め方（2026-08-07 ユーザー方針）

用語（CLAUDE.md 用語集）: 「オドメトリ基準の値」（例 `#T` rx/ry、相対・ドリフトする）
と「真の物理位置」（カメラで得る絶対真値）。その差が「真の物理誤差」。順番に:

1. **追従の改善**: 「オドメトリ基準の値」が計画パスにぴったり追従するよう改良
   （path_controller の追従精度・ゲイン）。= ログ上の rx/ry・hdg_err を小さくする。
2. **オドメトリと物理の一致**: 「オドメトリ基準」と「真の物理位置」が一致するよう
   オドメトリ精度を上げる（ホイール径・ジャイロスケールの校正）＋各種補正を実装
   （壁センサ値を用いた補正、壁切れ補正、壁上面位置補正など）。= 真の物理誤差を減らす。
3. **速度向上**: 上記の精度が確保できた段階で速度を上げる（本番 L2 の高速化）。

現状（2026-08-07、verify_loop ベースライン）: 追従は良好（max|hdg_err|≤2.64°・発振なし）
だが閉ループ誤差54.3mm（オドメトリ基準）を確認。→ まず1（追従）は概ねOK、2（校正・
補正）と3（速度）が主戦場。
