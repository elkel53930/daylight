...
北向き(スタート位置)へ復帰する(閉ループ誤差はログで測定)。俯瞰を投稿する。
次の走行も同じ (0,0) 北向きから始められる(置き直し不要)。
"""
...
def run_once(link, cruise: float, slalom: float, tag: str,
    ...
    # 走行後のスタート位置復帰: 完走時は南向き(rtheta≈-180°)なので、超信地旋回で
    # 北向き(=スタート位置、0,0 中心・北向き)へ戻す。向きは recenter のケーブル
    # よじれ対策(net_rotation_deg)で左右バランスを選ぶ(turn_to は TURN保持のまま
    # 返るため、完了後に stop で抜ける)。
    print("180°旋回でスタート位置(北向き)へ復帰します ...")
    recenter_ops.turn_to(link, 0.0)
    link.stop()
    print("スタート位置(北向き)へ復帰完了")
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        print("完走後は 180°旋回で (0,0) 中心・北向き(スタート位置)へ復帰。")
        return
