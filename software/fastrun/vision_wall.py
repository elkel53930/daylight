"""カメラ画像からの壁上面(赤帯)エッジ検出(fastrun移植版、2026-08-03)。

削除済み software/micromouse/vision.py(git d75afa7)をそのまま移植したもの。
純numpyで自己完結し、cv2に依存しない。走行前カメラ初期姿勢補正
(camera_align.py)から使う。
